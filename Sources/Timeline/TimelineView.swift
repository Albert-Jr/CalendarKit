...
public final class TimelineView: UIView {
    ...
    private func layoutEvents() {
        ...
        for (idx, attributes) in regularLayoutAttributes.enumerated() {
            ...
            eventView.updateWithDescriptor(event: descriptor, borderWidth: style.eventBorderWidth, borderColor: style.eventBorderColor, maximumNumberOfLines: style.eventMaximumNumberOfLines)
        }
    }
        ...
        for overlappingEvents in groupsOfEvents {
            ...
            for (index, event) in overlappingEvents.enumerated() {
                ...
                if style.eventsWillOverlap && totalCount > 1 {
                    ...
                    // Check if total fixed width would exceed calendar width
                    let totalFixedWidth = Double(totalCount - 1) * fixedWidth

                    if totalFixedWidth >= calendarWidth {
                        // Fall back to equal width distribution if fixed width would overflow
