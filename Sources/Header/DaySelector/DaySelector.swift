...
public final class DaySelector: UIView {
    ...
    public init(startDate: Date = Date(), daysInWeek: Int = 7) {
        ...
        initializeViews(viewType: DayColumnCell.self)
        configure()
    }
    ...
    override public func layoutSubviews() {
        ...
        let totalItemsWidth = size.width * itemCount + spacing * (itemCount - 1)
        let minX = (parentWidth - totalItemsWidth) / 2

        for (i, item) in items.enumerated() {

            var x = minX + (size.width + spacing) * Double(i)
